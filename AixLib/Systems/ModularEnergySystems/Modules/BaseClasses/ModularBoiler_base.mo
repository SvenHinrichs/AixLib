...
equation
  ...
  if Pump==false then
  connect(port_a, heatGeneratorNoControl.port_a);
  else
  end if;
  ...
  connect(switch7.y, fan1.y)
    annotation (Line(points={{-50,19},{-50,12},{-40,12}},
                                                 color={0,0,127}));
  connect(controlBoilerNotManufacturer.DeltaTWater_b, heatGeneratorNoControl.dTWater)
    annotation (Line(points={{-79,40.8},{-66,40.8},{-66,18},{-22,18},{-22,9},{
          -10,9}}, color={0,0,127}));
  ...
  connect(senTCold.T, controlBoilerNotManufacturer.TCold) annotation (Line(
        points={{-60,11},{-60,16},{-112,16},{-112,49},{-102,49}}, color={0,0,127}));
